...
/// SAF 경로 변환: content:// URI를 ffmpeg/ffprobe 입력으로 쓸 수 있는 형태로 변환
Future<String> _asFfmpegInputPath(String inputPath) async {
  if (inputPath.toLowerCase().startsWith('content://')) {
...
/// ffprobe JSON
Future<Map<String, dynamic>> _probeJson(String inputPath) async {
  final ip = await _asFfmpegInputPath(inputPath);
  final ipQ = _quoteArg(ip);
  final cmd = '-v error -of json -show_format -show_streams -i $ipQ';
  final sess = await FFprobeKit.execute(cmd);
  final out = await sess.getOutput();
  try {
...
/// 최후 폴백: dart_tags(APIC) (mp3, 로컬 경로만)
Future<String?> _extractCoverWithDartTags(String inputPath) async {
  try {
    if (!File(inputPath).existsSync()) {
      debugPrint('APIC fallback: file not found -> $inputPath');
...
Uint8List? _decodeFlacPictureFromBase64(String b64) {
  ...
  _readBeU32(bd, off);
  off += 4; // type
  final mimeLen = _readBeU32(bd, off);
  off += 4;
  if (raw.length < off + mimeLen) return null;
  off += mimeLen;

  final descLen = _readBeU32(bd, off);
  off += 4;
  if (raw.length < off + descLen) return null;
  off += descLen;

  off += 4 + 4 + 4 + 4; // width, height, depth, colors

  if (raw.length < off + 4) return null;
  final dataLen = _readBeU32(bd, off);
  off += 4;
  if (dataLen <= 0 || raw.length < off + dataLen) return null;
...
Future<String?> _extractOggCoverFromTags(Map<String, dynamic> allTags) async {
  final keys =
  allTags.keys.where((k) => '$k'.toLowerCase() == 'metadata_block_picture');
  if (keys.isEmpty) return null;
      ...
      if (img != null && img.isNotEmpty) {
        ...
        if (img.length >= 8 &&
          ...
          ext = 'png';
            ...
            utf8
                .decode(img.sublist(8, 12), allowMalformed: true)
                .toUpperCase() ==
                'WEBP') {
          ext = 'webp';
        }
...
Future<Map<String, String>> readAudioTags(
  ...
  if (isOgg) {
    final allLower = <String, dynamic>{};
    for (final e in formatTags.entries) {
      allLower[e.key.toLowerCase()] = e.value;
    }
    for (final e in streamTags.entries) {
      allLower[e.key.toLowerCase()] = e.value;
    }
    artworkPath = await _extractOggCoverFromTags(allLower);
    if (artworkPath != null) {
  ...
  // 4) ID3(APIC) 폴백 (content://일 경우 local File 접근 불가하므로 skip될 수 있음)
  if (artworkPath == null) {
    artworkPath = await _extractCoverWithDartTags(filePath);
  }
