...
void _assertCopySafe({
  ...
  switch (outContainer) {
    ...
    // MP3 컨테이너는 MP3 비트스트림만 안전 복사
      ok = (inCodec == 'mp3' || inCodec == 'mpga' || inCodec == 'mp2' */
/*희귀*//*
);
      break;
    case 'mp4':
...
*/
// lib/music/write_music_metadata.dart
import 'dart:convert';
import 'dart:io';
import 'dart:typed_data';
...
/// 참고:
/// - FFmpeg 라이선스/링킹 지침: https://ffmpeg.org/legal.html  (앱 배포 시 고지/소스 제공 의무 확인)  [ffmpeg 공식]
/// - Ogg/Vorbis/Opus 계열 커버아트는 VorbisComment에 FLAC PICTURE 블록을 base64로 싣는
///   'METADATA_BLOCK_PICTURE' 관례 사용.
/// - Opus/Vorbis/FLAC은 공개 표준/오픈 코덱으로 무료 사용이 가능하다는 점이 널리 문서화되어 있음
///   (IETF/Xiph).
...
/// SAF 읽기용 URL로 바꿔서 ffmpeg/ffprobe 입력으로 쓸 수 있게 한다.
Future<String> _asFfmpegInputPath(String inputPath) async {
  if (!inputPath.toLowerCase().startsWith('content://')) return inputPath;
  final safUrl = await FFmpegKitConfig.getSafParameterForRead(inputPath);
  if (safUrl == null || safUrl.isEmpty) {
...
Future<({String codec, String format})> _probeAudioInfo(String inputPath) async {
  // SAF 변환 포함
  final safeIn = await _asFfmpegInputPath(inputPath);
  final q = _quoteArg(safeIn);

  // ffprobe는 스트림/포맷 정보를 JSON으로 안정적으로 뽑아주므로
  // codec_name, format_name을 직접 읽을 수 있다. :contentReference[oaicite:3]{index=3}
  final cmd =
      '-v error '
      '-select_streams a:0 '
  ...
  final out = await sess.getOutput();
  if (out == null || out.trim().isEmpty) {
    throw Exception('입력 포맷 분석 실패(ffprobe 출력 없음)');
  }
  ...
  try {
    ...
    final format = (fmt['format_name'] ?? '').toString(); // 예: mp3, wav, ogg, mov,mp4,...

    if (codec.trim().isEmpty) {
      throw Exception('오디오 코덱을 찾지 못했습니다');
    }

    return (codec: codec.toLowerCase(), format: format.toLowerCase());
  } catch (e) {
    throw Exception('ffprobe JSON 파싱 실패: $e');
  }
}
...
Future<void> _encodeWithFfmpegAndPost({
  ...
  // 0) 입력 분석 + copy-only 안전검사
  final info = await _probeAudioInfo(inputPath);
  _assertCopySafe(inCodec: info.codec, outContainer: containerHint);
  ...
  if (hasStreamCover) {
    final ap = artworkPath!;
    if (ap.toLowerCase().startsWith('content://')) {
      coverIn = await FFmpegKitConfig.getSafParameterForRead(ap);
    } else {
      coverIn = ap;
    }
    if (coverIn == null || coverIn.isEmpty) coverIn = null;
